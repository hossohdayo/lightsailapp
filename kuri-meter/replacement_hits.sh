...
if [ $? -eq 0 ]; then
    ...
    echo "${TIME} [ERROR] 予期せぬエラーが発生 置換1異常終了" 2>&1 | tee -a "${LOG_FILE}"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "${TIME} [ERROR] 予期せぬエラーが発生 置換2異常終了" 2>&1 | tee -a "${LOG_FILE}"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "${TIME} [ERROR] 予期せぬエラーが発生 置換3異常終了" 2>&1 | tee -a "${LOG_FILE}"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "${TIME} [ERROR] 予期せぬエラーが発生 置換4異常終了" 2>&1 | tee -a "${LOG_FILE}"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "${TIME} [ERROR] 予期せぬエラーが発生 置換5異常終了" 2>&1 | tee -a "${LOG_FILE}"
    exit 1
fi
